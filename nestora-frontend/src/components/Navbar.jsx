...
const Navbar = () => {
    const {setShowLogin, user, logout, isOwner, axios, setOwner} = useAppContext()
  ...
  return (
    <div className={`h-14 flex items-center justify-between px-6 md:px-16 lg:px-24
    xl:px-32 py-4 text-gray-600 border-b border-borderColor relative transition-all
    ${location.pathname === '/' && 'bg-light'}`}>
        <Link to='/' className="flex items-center gap-2">
            <img src={assets.nestora_image} alt="logo" className="h-14 md:h-24 w-auto object-contain" />
        </Link>
        ...
        ${open ? 'max-sm:translate-x-0' : 'max-sm:translate-x-full'}`}>
            ...
            <div className='hidden lg:flex items-center text-sm gap-2 border border-borderColor px-3 rounded-full max-w-56'>
                ...
                outline-none placeholder:text-gray-500' placeholder="Search properties" />
